...
const evolutionRules={
  Caterpie:{level:7,to:"Metapod",kind:"bug"},Metapod:{level:10,to:"Butterfree",kind:"bird"},
  Pidgey:{level:18,to:"Pidgeotto",kind:"bird"},Pidgeotto:{level:36,to:"Pidgeot",kind:"bird"},
  Rattata:{level:20,to:"Raticate",kind:"rat"},Oddish:{level:21,to:"Gloom",kind:"plant"},Gloom:{level:32,to:"Vileplume",kind:"plant"},
  Zubat:{level:22,to:"Golbat",kind:"bat"},Gastly:{level:25,to:"Haunter",kind:"ghost"},Haunter:{level:38,to:"Gengar",kind:"ghost"},
  Sandshrew:{level:22,to:"Sandslash",kind:"rat"},Psyduck:{level:33,to:"Golduck",kind:"bird"},Magnemite:{level:30,to:"Magneton",kind:"ghost"},Growlithe:{level:30,to:"Arcanine",kind:"rat"}
};
...
let canvas,ctx,startScreen,battleActions,locationLabel,ballsLabel,caughtLabel,questLabel,levelLabel,expLabel,teamPanel,repeatTimer=null,evolutionNotice="";
...
function xpNeeded(level){return 18+level*8;}
function speciesKind(species){return encounters.find(item=>item.name===species)?.kind||evolutionRules[species]?.kind||"rat";}
function normalizeCaught(item){if(typeof item==="string")return{species:item,level:Math.max(3,gbc.partner.level-1),xp:0,kind:speciesKind(item),evolved:false};return{species:String(item?.species||item?.name||"Rattata"),level:Math.max(1,Number(item?.level)||3),xp:Math.max(0,Number(item?.xp)||0),kind:item?.kind||speciesKind(item?.species),evolved:Boolean(item?.evolved)};}
function load(){
  ...
  const savedBalls=Number(save.balls),savedPotions=Number(save.potions);gbc.balls=Number.isFinite(savedBalls)?Math.max(0,savedBalls):5;gbc.potions=Number.isFinite(savedPotions)?Math.max(0,savedPotions):3;gbc.caught=Array.isArray(save.caught)?save.caught.slice(0,80).map(normalizeCaught):[];gbc.profGift=Boolean(save.profGift);gbc.trainerDefeated=Boolean(save.trainerDefeated);gbc.shadowScoutDefeated=Boolean(save.shadowScoutDefeated);gbc.crystalFound=Boolean(save.crystalFound);gbc.questPhase=Math.max(0,Number(save.questPhase)||0);gbc.merchantGift=Boolean(save.merchantGift);gbc.completeAwarded=Boolean(save.completeAwarded);
}
...
function renderTeam(){if(!teamPanel)return;if(!gbc.caught.length){const empty=document.createElement("span");empty.className="gbc-team-empty";empty.textContent="Captura tu primer Pokémon en la hierba alta.";teamPanel.replaceChildren(empty);return;}teamPanel.replaceChildren(...gbc.caught.slice(0,12).map(mon=>{const card=document.createElement("div"),name=document.createElement("strong"),meta=document.createElement("span"),bar=document.createElement("i"),fill=document.createElement("b");card.className=`gbc-mon${mon.evolved?" evolved":""}`;name.textContent=mon.species;meta.textContent=`Nv.${mon.level} · EXP ${mon.xp}/${xpNeeded(mon.level)}`;fill.style.width=`${Math.min(100,mon.xp/xpNeeded(mon.level)*100)}%`;bar.append(fill);card.append(name,meta,bar);return card;}));}
function updateStatus(){ballsLabel.textContent=String(gbc.balls);caughtLabel.textContent=String(gbc.caught.length);if(levelLabel)levelLabel.textContent=String(gbc.partner.level);if(expLabel)expLabel.textContent=`${gbc.partner.xp}/${xpNeeded(gbc.partner.level)}`;if(questLabel)questLabel.textContent=questText();renderTeam();const p=gbc.player;let name="🛤️ Ruta Verde";if(p.x>=17&&p.x<=38&&p.y<=12)name="🏘️ Ciudad Pixel";else if(p.y>=27&&p.x<24)name="🌲 Bosque Voltio";else if(p.x>=40&&p.y>=22)name="🏜️ Desierto Ámbar";else if(p.x>=43&&p.y<21)name="⛰️ Sierra Prisma";else if(p.x>=25&&p.x<41&&p.y>=18)name="💧 Lago Celeste";else if(map[p.y]?.[p.x]==="grass")name="🌿 Pradera Salvaje";locationLabel.textContent=name;}
...
function spritePokemon(kind,x,y,scale=4){
  ...
  ctx.fillStyle="rgba(29,49,34,.2)";ctx.fillRect(x+2*scale,y+15*scale,13*scale,2*scale);
...
function expBar(x,y,w,xp,level){ctx.fillStyle="#314256";ctx.fillRect(x,y,w,6);ctx.fillStyle="#55b9e8";ctx.fillRect(x+1,y+1,(w-2)*Math.min(1,xp/xpNeeded(level)),4);}
function drawBattleBackdrop(){ctx.fillStyle="#98c777";ctx.fillRect(0,0,640,330);ctx.fillStyle="#b8dd8c";ctx.fillRect(0,0,640,92);ctx.fillStyle="#7bac62";ctx.fillRect(0,92,640,28);for(let x=0;x<640;x+=32){ctx.fillStyle=x%64?"#8dbb6c":"#83b163";ctx.fillRect(x,248,32,82);ctx.fillStyle="#cbe8a2";ctx.fillRect(x+7,74+(x%96)/12,16,4);}ctx.fillStyle="#668e58";for(let x=6;x<640;x+=46)ctx.fillRect(x,272+(x%3)*8,19,5);}
function drawBattle(){const b=gbc.battle;drawBattleBackdrop();ctx.fillStyle="#d8e2a5";ctx.fillRect(20,46,285,76);ctx.strokeStyle=palette.ink;ctx.lineWidth=4;ctx.strokeRect(20,46,285,76);ctx.fillStyle=palette.ink;ctx.font="bold 19px monospace";ctx.fillText(`${b.enemy.name}  Nv.${b.enemy.level}`,35,76);ctx.font="bold 13px monospace";ctx.fillText("PS",35,101);hpBar(70,94,210,b.enemy.hp,b.enemy.maxHp);spritePokemon(b.enemy.kind,455,125,6);spritePokemon("pikachu",85,190,6);ctx.fillStyle="#d8e2a5";ctx.fillRect(330,218,290,105);ctx.strokeRect(330,218,290,105);ctx.fillStyle=palette.ink;ctx.font="bold 18px monospace";ctx.fillText(`PIKACHU  Nv.${gbc.partner.level}`,345,246);ctx.font="bold 13px monospace";ctx.fillText(`PS ${gbc.partner.hp}/${gbc.partner.maxHp}`,345,271);hpBar(345,282,245,gbc.partner.hp,gbc.partner.maxHp);ctx.fillText("EXP",345,306);expBar(386,300,204,gbc.partner.xp,gbc.partner.level);ctx.fillStyle=palette.paper;ctx.fillRect(12,335,616,132);ctx.strokeRect(12,335,616,132);ctx.fillStyle=palette.ink;ctx.font="bold 17px monospace";wrapText(b.message,30,368,570,25);}
...
function finishBattle(message){gbc.mode="world";gbc.battle=null;gbc.busy=false;battleActions.hidden=true;save();checkCompletion();const extra=evolutionNotice?` ${evolutionNotice}`:"";evolutionNotice="";showDialog(message+extra);updateStatus();draw();}
function evolveCaptured(mon){const evolutions=[];let guard=0;while(evolutionRules[mon.species]&&mon.level>=evolutionRules[mon.species].level&&guard++<3){const from=mon.species,rule=evolutionRules[from];mon.species=rule.to;mon.kind=rule.kind;mon.evolved=true;evolutions.push(`${from} → ${mon.species}`);}return evolutions;}
function gainXp(amount){let partnerLeveled=false;gbc.partner.xp+=amount;while(gbc.partner.xp>=xpNeeded(gbc.partner.level)){gbc.partner.xp-=xpNeeded(gbc.partner.level);gbc.partner.level++;gbc.partner.maxHp+=4;gbc.partner.hp=gbc.partner.maxHp;partnerLeveled=true;}const evolutionMessages=[];for(const mon of gbc.caught){mon.xp+=Math.max(1,Math.floor(amount*.55));while(mon.xp>=xpNeeded(mon.level)){mon.xp-=xpNeeded(mon.level);mon.level++;}evolutionMessages.push(...evolveCaptured(mon));}if(evolutionMessages.length){tone(1060,.18,"square",.03);evolutionNotice=`✨ ¡EVOLUCIÓN! ${evolutionMessages.join(" · ")}.`;}updateStatus();save();return partnerLeveled;}
...
function battleAction(action){
  ...
  else if(action==="ball"){if(enemy.trainer){b.message="No puedes capturar la criatura de otro entrenador.";draw();return;}if(gbc.balls<=0){b.message="No te quedan Pokéballs. Visita al Profesor Pixel.";draw();return;}gbc.balls--;updateStatus();const weak=1-enemy.hp/enemy.maxHp,chance=Math.min(.94,enemy.catchRate+weak*.5);gbc.busy=true;b.message=`¡Lanzaste una Pokéball! Probabilidad ${Math.round(chance*100)}%...`;sfx("coin");draw();setTimeout(()=>{if(Math.random()<chance){gbc.caught.push({species:enemy.name,level:enemy.level,xp:0,kind:enemy.kind,evolved:false});gainXp(6+enemy.level*2);award(7,6,`${enemy.name} capturado`);finishBattle(`¡Genial! ${enemy.name} fue capturado en Nv.${enemy.level}. Ahora ganará EXP junto a Pikachu.`);}else{b.message=`¡${enemy.name} se escapó de la Pokéball! Debilítalo más para aumentar la probabilidad.`;gbc.busy=false;draw();setTimeout(enemyTurn,420);}},720);}
